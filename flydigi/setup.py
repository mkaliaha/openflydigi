...
DESKTOP_NAME = "openflydigi.desktop"
DESKTOP_DIR = os.path.join(
    os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share"),
    "applications")
DESKTOP_PATH = os.path.join(DESKTOP_DIR, DESKTOP_NAME)

# The name the entry shipped under before the app was called OpenFlydigi.
# Installing removes it, for the same reason STALE_RULES exists below: two
# entries in the menu, both starting the same window, is worse than one.
STALE_DESKTOP = os.path.join(DESKTOP_DIR, "flydigi-apex5.desktop")
...
[Unit]
Description=OpenFlydigi adaptive-trigger daemon
...
def desktop_text():
    ...
    box = container_name()
    comment = ("Configure a Flydigi controller"
...
def install_desktop():
    ...
    # The entry this one replaces. Unprivileged and in the same directory, so
    # unlike STALE_RULES it needs no escalation and can go here.
    if STALE_DESKTOP != DESKTOP_PATH:
        try:
            os.unlink(STALE_DESKTOP)
        except OSError:
            pass
...
def remove_desktop():
    for path in (DESKTOP_PATH, STALE_DESKTOP):
        try:
            os.unlink(path)
        except OSError:
            pass
